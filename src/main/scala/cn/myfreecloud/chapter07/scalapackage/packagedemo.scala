...
package cn.myfreecloud {
  ...
  //2. package object scala 表示创建一个包对象 scala, 他是 cn.myfreecloud.scala这个包对应的包对象
  //3. 每一个包都可以有一个包对象
  //4. 包对象的名字需要和子包一样
  //5. 在包对象中可以定义变量，方法
  ...
  package scala { //包 cn.myfreecloud.scala

    class Person { // 表示在 cn.myfreecloud.scala下创建类 Person
      val name = "Nick"
    ...
    object Test100 { //表示在 cn.myfreecloud.scala 创建object Test
